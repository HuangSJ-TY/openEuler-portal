import bgLight from '../img/banner-pc-light.jpg';
import bgDark from '../img/banner-pc-dark.jpg';
import bgMbLight from '../img/banner-mb-light.jpg';
import bgMbDark from '../img/banner-mb-dark.jpg';

import textLight from '../img/textImg-pc-light.png';
import textDark from '../img/textImg-pc-dark.png';
import textMbLight from '../img/textImg-mb-light.png';
import textMbDark from '../img/textImg-mb-dark.png';

import openEuler from '../img/logo/openEuler_light.png';
import openEulerDark from '../img/logo/openEuler_dark.png';
import huawei from '../img/logo/huawei_light.png';
import huawei_dark from '../img/logo/huawei_dark.png';
import qiling from '../img/logo/qiling_light.png'
import qiling_dark from '../img/logo/qiling_dark.png';
import xinan from '../img/logo/xinan_light.png';
import xinan_dark from '../img/logo/xinan_dark.png';
import ningsi from '../img/logo/ningsi_light.png';
import ningsi_dark from '../img/logo/ningsi_dark.png';
import xfusion from '../img/logo/xfusion_light.png';
import xfusion_dark from '../img/logo/xfusion_dark.png';
import dongfang from '../img/logo/dongfang_light.png';
import dongfang_dark from '../img/logo/dongfang_dark.png';
import ruantong from '../img/logo/ruantong_light.png';
import ruantong_dark from '../img/logo/ruantong_dark.png';
import runhe from '../img/logo/runhe_light.png';
import runhe_dark from '../img/logo/runhe_dark.png';
import baolande from '../img/logo/baolande_light.png';
import baolande_dark from '../img/logo/baolande_dark.png';
import tongxin from '../img/logo/tongxin_light.png';
import tongxin_dark from '../img/logo/tongxin_dark.png';

import xiexiuqi from '../img/guest/xiexiuqi.png';
import luoyukai from '../img/guest/luoyukai.png';
import dukaitian from '../img/guest/dukaitian.png';
import lichenbin from '../img/guest/lichenbin.png';
import tanggeliang from '../img/guest/tanggeliang.png';
import zhutao from '../img/guest/zhutao.png';
import zhangyizheng from '../img/guest/zhangyizheng.png';
import chenxin from '../img/guest/chenxin.png';
import zhuzexu from '../img/guest/zhuzexu.png';
import liyang from '../img/guest/liyang.png';

import dudongdong from '../img/guest/dudongdong.png';
import lujun from '../img/guest/lujun.png';
import zhangtao from '../img/guest/zhangtao.png';
import jianghaoran from '../img/guest/jianghaoran.png';
import duanchenghao from '../img/guest/duanchenghao.png';
import tangyouling from '../img/guest/tangyouling.png';
import songshi from '../img/guest/songshi.png';
import yaoguangzhong from '../img/guest/yaoguangzhong.png';
import yanglu from '../img/guest/yanglu.png';
import zhangrui from '../img/guest/zhangrui.png';
import wangwenkuan from '../img/guest/wangwenkuan.png';
import cryolitia from '../img/guest/cryolitia.png';
import songjiayu from '../img/guest/songjiayu.png';
import renwei from '../img/guest/renwei.png';
import wangjingwei from '../img/guest/wangjingwei.png';
import jinxiaoxu from '../img/guest/jinxiaoxu.png';
import zhangxiao from '../img/guest/zhangxiao.png';
import zhangzehua from '../img/guest/zhangzehua.png';
import sunzhenghua from '../img/guest/sunzhenghua.png';
import zhouqiang from '../img/guest/zhouqiang.png';
import liubingwei from '../img/guest/liubingwei.png';

import caixinqi from '../img/guest/caixinqi.png';
import zengyi from '../img/guest/zengyi.png';
import dingjiahui from '../img/guest/dingjiahui.png';
import feijianqiang from '../img/guest/feijianqiang.png';
import hukuang from '../img/guest/hukuang.png';
import jiangdongping from '../img/guest/jiangdongping.png';
import jinlong from '../img/guest/jinlong.png';
import lichaoyang from '../img/guest/lichaoyang.png';
import lipeng from '../img/guest/lipeng.png';
import liangyi from '../img/guest/liangyi.png';
import wujian from '../img/guest/wujian.png';
import liubaoyu from '../img/guest/liubaoyu.png';
import luweijun from '../img/guest/luweijun.png';
import tanrui from '../img/guest/tanrui.png';
import wanfeng from '../img/guest/wanfeng.png';
import wangdawei from '../img/guest/wangdawei.png';
import wangwei from '../img/guest/wangwei.png';
import wangxiaoyun from '../img/guest/wangxiaoyun.png';
import wangzhiqiang from '../img/guest/wangzhiqiang.png';
import weijiangang from '../img/guest/weijiangang.png';
import wuheng from '../img/guest/wuheng.png';
import wuzhiguo from '../img/guest/wuzhiguo.png';
import xulei from '../img/guest/xulei.png';
import xupeiwen from '../img/guest/xupeiwen.png';
import yangbin from '../img/guest/yangbin.png';
import yunlipeng from '../img/guest/yunlipeng.png';
import zhuowenjie from '../img/guest/zhuowenjie.png';
import zhongmaoheng from '../img/guest/zhongmaoheng.png';
import zhongtao from '../img/guest/zhongtao.png';
import liuxinliang from '../img/guest/liuxinliang.png';

import chengxiaoming from '../img/guest/chengxiaoming.png';
import niguangnan from '../img/guest/niguangnan.png';
import penghonghua from '../img/guest/penghonghua.png';
import xiongwei from '../img/guest/xiongwei.png';
import yangchaobin from '../img/guest/yangchaobin.png';
import yangmingming from '../img/guest/yangmingming.png';
import zengwenbin from '../img/guest/zengwenbin.png';
import shaoguanglu from '../img/guest/shaoguanglu.png';
import duhai from '../img/guest/duhai.png';

import andrew from '../img/guest/andrew.png';
import chenyouxuan from '../img/guest/chenyouxuan.png';
import tangxiaolei from '../img/guest/tangxiaolei.png';
import gaolijiang from '../img/guest/gaolijiang.png';
import grant from '../img/guest/grant.png';
import huchunming from '../img/guest/huchunming.png';
import huxinwei from '../img/guest/huxinwei.png';
import linzhenfu from '../img/guest/linzhenfu.png';
import yeqinglong from '../img/guest/yeqinglong.png';

import andy from '../img/guest/andy.png';
import heliyun from '../img/guest/heliyun.png';
import liping from '../img/guest/liping.png';
import wanglin from '../img/guest/wanglin.png';
import wangyucheng from '../img/guest/wangyucheng.png';
import wuchenggang from '../img/guest/wuchenggang.png';
import xinxiaohui from '../img/guest/xinxiaohui.png';
import zhangchi from '../img/guest/zhangchi.png';
import fengyan from '../img/guest/fengyan.png';
import hanyuankun from '../img/guest/hanyuankun.png';
import huangjun from '../img/guest/huangjun.png';
import lijingjing from '../img/guest/lijingjing.png';
import liyuzhen from '../img/guest/liyuzhen.png';
import sunyanan from '../img/guest/sunyanan.png';
import zhegnchenhui from '../img/guest/zhegnchenhui.png';

export default {
  banner: {
    bg: {
      light: bgLight,
      dark: bgDark,
    },
    bgMb: {
      light: bgMbLight,
      dark: bgMbDark,
    },
    textImg: {
      light: textLight,
      dark: textDark,
    },
    textImgMb: {
      light: textMbLight,
      dark: textMbDark,
    },
    signUpTitle: 'Register',
    link: {
      pc: 'https://openatomcon.openatom.cn/registration/?code=Nhvt&activityNo=HD20251013162127',
      mo: 'https://openatomcon.openatom.cn/registration_mobile/?code=Nhvt&activityNo=HD20251013162127',
    },
  },
  introduce: {
    desc: [
      'Over the past six years, openEuler has grown into a leading open source operating system (OS), supporting diversified computing power and powering core applications across industries such as Internet, finance, and carrier.',
      'Looking ahead, openEuler will continue to uphold the vision of "AI for OS, OS for AI", paving the way for a new era of OS evolution, where AI and OS converge to drive AI computing. ',
      'On November 14–15, 2025, the Operating System Conference & openEuler Summit 2025 will take place at the ZhongGuanCun International Innovation Center in Beijing. At the event, openEuler will launch a major AI-driven version, and global partners will unveil flagship products built on openEuler — showcasing shared innovation and confidence in advancing next-gen OS for the intelligent era.'
    ],
    tips: 'At the event, you will:',
    list: [
      'Be the first to witness our next-gen OS and discover what\'s next in OS evolution.',
      'Experience openEuler\'s partner product releases for the next globalization chapter.',
      'Join 30+ keynotes, sessions, and roundtable to exchange ideas with experts and developers worldwide, and hear cutting-edge insights and real-world success stories.',
      'Visit the 1,000㎡ exhibition area to explore the latest technological innovations, engage in hands-on activities, and immerse yourself in the collaborative achievements.'
    ],
    slogan: [
      'Let\'s meet in Beijing — to inspire, collaborate, and co-create the next chapter of the global open source ecosystem.'
    ]
  },
  live: {
    title: 'Live',
    list: [
      {
        time: 'Nov 14',
        lives: [
          {
            liveId: 15926,
            liveTestId: 15912,
            id: 0,
            name: 'Operating System Conference Keynote',
          },
          {
            liveId: 15928,
            liveTestId: 15914,
            name: 'Science & Education Innovation',
          },
          {
            liveId: 15929,
            liveTestId: 15915,
            name: 'Intelligent Manufacturing Industry',
          },
          {
            liveId: 15930,
            liveTestId: 15916,
            name: 'KylinSoft',
          },
          {
            liveId: 15931,
            liveTestId: 15917,
            name: 'Kylinsec',
          },
        ]
      },
      {
        time: 'Nov 15',
        lives: [
          {
            liveId: 15927,
            liveTestId: 15913,
            id: 0,
            name: 'openEuler Summit Keynote',
          },
          {
            liveId: 15932,
            liveTestId: 15918,
            name: 'Kernel & All-Scenario',
          },
          {
            liveId: 15933,
            liveTestId: 15919,
            name: 'Security & Confidential Computing',
          },
          {
            liveId: 15934,
            liveTestId: 15920,
            name: 'AI',
          },
          {
            liveId: 15935,
            liveTestId: 15921,
            name: 'Cloud Native',
          },
          {
            liveId: 15936,
            liveTestId: 15922,
            name: 'UnifiedBus',
          },
          {
            liveId: 15937,
            liveTestId: 15923,
            name: 'Embedded & Embodied AI',
          },
          {
            liveId: 15938,
            liveTestId: 15924,
            name: 'Developer Enablement & Tools',
          },
          {
            liveId: 15939,
            liveTestId: 15925,
            name: 'RISC-V',
          },
        ],
      }
    ]
  },
  agent: {
    title: 'Agenda',
    tips: 'The agenda will be updated soon. Stay tuned!',
    text: 'Click here for details',
    list: [
      {
        date: "14",
        children: [
          {
            title: 'Keynote',
            children: [
              {
                time: '09:30 - 09:50',
                content: [
                  {
                    title: 'Welcome & Opening Remarks',
                    name: 'Guest speaker',
                    post: '',
                  },
                ]
              },
              {
                time: '09:50 - 10:55',
                content: [
                  {
                    title: 'Intelligence Unbound: openEuler, an OS Built for AI',
                    name: 'Dr. Xiong Wei',
                    post: 'Chairperson, OpenAtom openEuler Committee',
                  },
                  {
                    title: 'Internet × openEuler: Advancing AI Innovations for Industries',
                    guest: [
                      {
                        name: 'Du Hai',
                        post: 'General Manager of Hybrid Cloud, Baidu AI Cloud',
                      },
                      {
                        name: 'Zeng Wenbin',
                        post: 'Director of Operating System, Meituan',
                      }
                    ]
                  },
                  {
                    title: 'China Telecom × openEuler: Building AI-powered Digital Infrastructure',
                    name: 'Jiang Biao',
                    post: ' Chief Expert of Cloud Computing and Computing Power, China Telecom',
                  },
                  {
                    title: '2025 openEuler Practitioners and Practices Award',
                    name: 'User representatives',
                    post: '',
                  }
                ]
              },
              {
                time: '10:55 - 11:25',
                content: [
                  {
                    title: 'Global Carrier × openEuler: Building a Locally Rooted Digital Ecosystem',
                    name: 'Guest speaker',
                    post: '',
                  },
                  {
                    title: 'Phoenix Contact × openEuler: Accelerating the OT-IT Convergence',
                    name: 'Guest speaker',
                    post: '',
                  },
                  {
                    title: 'openEuler Global Pioneer Partners Award',
                    name: 'Partner representatives',
                    post: '',
                  }
                ]
              },
            ],
            tab: []
          },
          {
            title: 'Subforums',
            children: [],
            tab: [
              {
                label: 'KylinSoft',
                time: '13:30 - 15:45',
                address: '',
                children: [
                  {
                    time: '13:30-13:40',
                    content: [
                      {
                        title: 'Welcome & Opening Remarks',
                        name: 'Guest speaker',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '13:40-13:50',
                    content: [
                      {
                        title: 'Welcome & Opening Remarks',
                        name: 'Guest speaker',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '13:50-14:05',
                    content: [
                      {
                        title: 'Embracing Open Source × AI: KylinOS Integrated Solution for Servers',
                        name: 'Hou Jian',
                        post: 'Deputy General Manager, Server R&D Center, KylinSoft',
                      }
                    ]
                  },
                  {
                    time: '14:05-14:20',
                    content: [
                      {
                        title: 'When AI Meets Archiving',
                        name: 'Xia Guangjun',
                        post: 'Deputy General Manager, PDE',
                      }
                    ]
                  },
                  {
                    time: '14:20-14:35',
                    content: [
                      {
                        title: 'Building AI-powered Digital Campus with Newcapec Foundation Model',
                        name: 'Zhang Weibing',
                        post: 'Solution Director, Newcapec AI Research Institute ',
                      }
                    ]
                  },
                  {
                    time: '14:35-14:50',
                    content: [
                      {
                        title: 'How Heterogeneous Compute is Powering Autonomous Innovation',
                        name: 'Wu Yuebo',
                        post: 'Solution Director, CEC Great Wall',
                      }
                    ]
                  },
                  {
                    time: '14:50-15:05',
                    content: [
                      {
                        title: 'Redefining Office Productivity: The New Homegrown Device-Edge-Cloud Stack',
                        name: 'Jia Anya',
                        post: 'Senior Product Director, SenseTime',
                      }
                    ]
                  },
                  {
                    time: '15:05-15:20',
                    content: [
                      {
                        title: 'AI Appliance Deployments in Private Domains',
                        name: 'Liu Yanyan',
                        post: 'Chief Architect, SuperCloud',
                      }
                    ]
                  },
                  {
                    time: '15:20-15:35',
                    content: [
                      {
                        title: 'AI-COP: A New Intelligent Ecosystem Paradigm',
                        name: 'Guan Junjun',
                        post: 'Director of Ecosystem Cooperation, Seeyon',
                      }
                    ]
                  },
                  {
                    time: '15:35-15:45',
                    content: [
                      {
                        title: 'Luck Draw',
                        name: '',
                        post: '',
                      }
                    ]
                  }
                ],
              },
              {
                label: 'Kylinsec',
                time: '14:30 - 17:00',
                address: '',
                children: [
                  {
                    time: '14:30-14:35',
                    content: [
                      {
                        title: 'Welcome & Opening Remarks',
                        name: 'Dr. Xiong Wei',
                        post: 'Chairperson, OpenAtom openEuler Committee',
                      }
                    ]
                  },
                  {
                    time: '14:35-14:45',
                    content: [
                      {
                        title: 'Welcome & Opening Remarks',
                        name: 'Yang Tao',
                        post: 'Chairman, Kylinsec',
                      }
                    ]
                  },
                  {
                    time: '14:45-15:05',
                    content: [
                      {
                        title: 'Kylinsec OS V6 Launch',
                        name: 'Guest speaker',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '15:05-15:25',
                    content: [
                      {
                        title: 'Kylinsec Robot OS Launch',
                        name: 'Guest speaker',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '15:25-15:45',
                    content: [
                      {
                        title: 'Kylinsec AI Cloud Product Launch',
                        name: 'Guest speaker',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '15:45-16:00',
                    content: [
                      {
                        title: 'Kylinsec Applications in AI Computing',
                        name: 'Guest speaker',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '16:00-16:15',
                    content: [
                      {
                        title: 'Kylinsec Applications in Manufacturing',
                        name: 'Guest speaker',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '16:15-16:30',
                    content: [
                      {
                        title: 'Kylinsec Applications in Electric Power',
                        name: 'Guest speaker',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '16:30-16:45',
                    content: [
                      {
                        title: 'Kylinsec Applications in Finance',
                        name: 'Guest speaker',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '16:45-17:00',
                    content: [
                      {
                        title: 'Kylinsec Applications in Intranet Virtualization',
                        name: 'Guest speaker',
                        post: '',
                      }
                    ]
                  }
                ],
              },
              {
                label: 'Internet Roundtable',
                time: '14:00 - 17:00',
                address: '',
                children: [
                  {
                    time: '14:00-14:05',
                    content: [
                      {
                        title: 'Welcome & Opening Remarks',
                        name: 'Guest speaker',
                        post: '',
                      },
                    ]
                  },
                  {
                    time: '14:05-14:10',
                    content: [
                      {
                        title: 'Advancing openEuler in Internet for Community Growth',
                        name: 'Guest speaker',
                        post: '',
                      },
                    ]
                  },
                  {
                    time: '14:10-15:10',
                    content: [
                      {
                        title: 'UnifiedBus SuperPoD in Internet: Practices and Value Scenarios',
                        name: 'Guest speaker',
                        post: '',
                      },
                    ]
                  },
                  {
                    time: '15:10-15:50',
                    content: [
                      {
                        title: 'AI Computing Cluster in Internet: Practices',
                        name: 'Guest speaker',
                        post: '',
                      },
                    ]
                  },
                  {
                    time: '15:50-16:30',
                    content: [
                      {
                        title: 'Multi-core High-density Computing in Internet: Practices',
                        name: 'Guest speaker',
                        post: '',
                      },
                    ]
                  },
                  {
                    time: '16:30-17:00',
                    content: [
                      {
                        title: 'openEuler × Kunpeng in Internet: Practices',
                        name: 'Guest speaker',
                        post: '',
                      },
                    ]
                  },
                ]
              },
              {
                label: 'openEuler Global Ecosystem',
                time: '13:30 - 15:00',
                address: '',
                children: [
                  {
                    time: '13:30-13:35',
                    content: [
                      {
                        title: 'Welcome & Opening Remarks',
                        name: 'Fu Haiwei',
                        post: 'Director, Business Development Department, OpenAtom Foundation',
                      }
                    ]
                  },
                  {
                    time: '13:35-13:50',
                    content: [
                      {
                        title: 'Remarks by Hardware Vendors',
                        guest: [
                          {
                            name: 'Andrew Wafaa',
                            post: 'Arm Senior Director Software Communities & Fellow, PyTorch Board',
                          },
                          {
                            name: 'Liu Bingwei',
                            post: 'Senior Director, Software Technology Group, Intel China',
                          },
                          {
                            name: 'Wang Wenkuan',
                            post: 'Linux Kernel Expert, AMD China',
                          },
                        ]
                      }
                    ]
                  },
                  {
                    time: '13:50-14:00',
                    content: [
                      {
                        title: 'Open Source Software Meets AI: The Road Ahead',
                        name: 'Dr. Xiong Wei',
                        post: 'Chairperson, OpenAtom openEuler Committee',
                      }
                    ]
                  },
                  {
                    time: '14:00-14:15',
                    content: [
                      {
                        title: 'Intelligent Transformation Practices in Carrier',
                        name: 'Yang Yuchao',
                        post: 'Senior Technical Expert, Overseas Business, China Telecom eSurfing Cloud',
                      }
                    ]
                  },
                  {
                    time: '14:15-14:30',
                    content: [
                      {
                        title: 'Intelligent Transformation Practices in Internet',
                        name: 'Zhang Zehua',
                        post: 'Director, Ads Algorithm, JD.com',
                      }
                    ]
                  },
                  {
                    time: '14:30-14:40',
                    content: [
                      {
                        title: 'AI Training & Inference Application Cases',
                        name: 'Zhang Xiao',
                        post: 'Director, Intelligent Computing Platform, iFLYTEK Spark AI Research Institute',
                      }
                    ]
                  },
                  {
                    time: '14:40-14:50',
                    content: [
                      {
                        title: 'ERP Application Cases',
                        name: 'Jin Xiaoxu',
                        post: 'Director of Chinese Enterprise Go Global, Yonyou',
                      }
                    ]
                  },
                  {
                    time: '14:50-15:00',
                    content: [
                      {
                        title: 'openEuler Global Service System',
                        name: 'Paul Zhou',
                        post: 'Senior Delivery Director, Wind River',
                      }
                    ]
                  },
                ],
              },
              {
                label: 'Science & Education Innovation',
                time: '13:30 - 16:30',
                address: '',
                children: [
                  {
                    time: '13:30-13:40',
                    content: [
                      {
                        title: 'Welcome & Opening Remarks',
                        guest: [
                          {
                            name: 'Hu Xinwei',
                            post: 'Technical Committee Chairperson, OpenAtom openEuler',
                          },
                          {
                            name: 'Hu Chunming',
                            post: ' Dean, School of Software, Beihang University',
                          }
                        ]
                      }
                    ]
                  },
                  {
                    time: '13:40-15:00',
                    content: [
                      {
                        title: 'Key Technologies of Heterogeneous Pooling System Based on High-speed Interconnect Bus',
                        name: 'Zhao Xinkui',
                        post: 'Researcher of 100-Young Professor Plan, School of Software Technology, Zhejiang University',
                      },
                      {
                        title: 'Parameter Storage System for Recommendation Models',
                        name: 'Xie Minhui',
                        post: 'Lecturer, Department of Computer Science, School of Information, Renmin University of China',
                      },
                      {
                        title: 'End-to-End Complex Attack Detection in AI',
                        name: 'Li Ding',
                        post: 'Assistant Professor, Peking University',
                      },
                      {
                        title: 'Resource Management and Reliability Assurance for Heterogeneous AI Infra System Software',
                        name: 'Yang Renyu',
                        post: 'Associate Professor, Beihang University',
                      }
                    ]
                  },
                  {
                    time: '15:00-15:05',
                    content: [
                      {
                        title: 'Awards Ceremony: OSPP 2025 Star Projects',
                        name: '',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '15:05-15:20',
                    content: [
                      {
                        title: 'Stories of Outstanding Student Developers',
                        name: 'Guest speaker',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '15:20-15:50',
                    content: [
                      {
                        title: 'Open Source in Education: From Campus Initiatives to Talent Assessment',
                        name: 'Han Jiang',
                        post: 'Head of Education Department, OpenAtom Foundation',
                      },
                      {
                        title: 'Curriculum Development and Industry-Academia Collaboration in System Software',
                        name: 'Zhang Li',
                        post: 'Professor, Beihang University',
                      }
                    ]
                  },
                  {
                    time: '15:50-16:30',
                    content: [
                      {
                        title: 'Collaborating in Open Source to Bridge Academia and Industry for a Prosperous Ecosystem',
                        name: 'Hu Chunming',
                        post: 'Dean, School of Software, Beihang University',
                      }
                    ]
                  }
                ],
              },
              {
                label: 'Intelligent Manufacturing Industry',
                time: '13:30 - 16:20',
                address: '',
                children: [
                  {
                    time: '13:30-13:40',
                    content: [
                      {
                        title: 'Welcome & Opening Remarks',
                        name: 'Liu Cheng',
                        post: 'Product Planning Expert, openEuler',
                      }
                    ]
                  },
                  {
                    time: '13:40-14:00',
                    content: [
                      {
                        title: 'PLCnext Technology in Smart Manufacturing',
                        name: 'Yang Lu',
                        post: 'Director of PLCnext Software Application, Phoenix Contact',
                      }
                    ]
                  },
                  {
                    time: '14:00-14:20',
                    content: [
                      {
                        title: 'Foundational Technology for Industrial Control in Intelligent Automation',
                        name: 'Wang Zhiqiang',
                        post: 'Head of Marketing, CODESYS Software Group',
                      }
                    ]
                  },
                  {
                    time: '14:20-14:40',
                    content: [
                      {
                        title: 'An openEuler OS for Lights-out Labs (Factories) in Large Scientific Facilities',
                        name: 'Lu Jun',
                        post: 'Deputy Director, Lineage Engineering Technology Center, Guangzhou Institutes of Biomedicine and Health, Chinese Academy of Sciences',
                      }
                    ]
                  },
                  {
                    time: '14:40-15:00',
                    content: [
                      {
                        title: 'Real-time Industry OS for High-end CNC Systems: Requirement Changes',
                        name: 'Song Shi',
                        post: 'Chief Engineer, GSK CNC Equipment',
                      }
                    ]
                  },
                  {
                    time: '15:00-15:20',
                    content: [
                      {
                        title: 'Embodied AI Robotics: Exploration and Practices',
                        name: 'Zeng Yi',
                        post: 'Director of Robotics Market, Topstar',
                      }
                    ]
                  },
                  {
                    time: '15:20-15:40',
                    content: [
                      {
                        title: 'openEuler Embedded Industrial Control in Smart Transportation ',
                        name: 'Wang Wei ',
                        post: 'Head of Unmanned Equipment, JRLC',
                      }
                    ]
                  },
                  {
                    time: '15:40-16:00',
                    content: [
                      {
                        title: 'Software Empowers Industrial Intelligence',
                        name: 'Yao Guangzhong',
                        post: 'Technology Expert, Henan KunLun Technology',
                      }
                    ]
                  },
                  {
                    time: '16:00-16:20',
                    content: [
                      {
                        title: 'openEuler Embedded × Ascend 310B for Edge AI',
                        name: 'Zhang Tao',
                        post: 'Manager of Embedded Dept, HopeRun',
                      }
                    ]
                  }
                ],
              },
            ]
          }
        ]
      },
      {
        date: "15",
        children: [
          {
            title: 'Keynote',
            children: [
              {
                time: '09:30 - 09:55',
                content: [
                  {
                    title: 'openEuler Community Development',
                    guest: [
                      {
                        name: 'Dr. Xiong Wei',
                        post: 'Chairperson, OpenAtom openEuler Committee',
                      },
                      {
                        name: 'Tang Xiaolei',
                        post: 'Vice President, AMD',
                      },
                      {
                        name: 'Andrew Wafaa',
                        post: 'Arm Senior Director Software Communities & Fellow, PyTorch Board',
                      },
                      {
                        name: 'Grant Likely',
                        post: 'CTO, Linaro',
                      }
                    ]
                  }
                ]
              },
              {
                time: '09:55 - 10:30',
                content: [
                  {
                    titles: ['openEuler Technology Trends and Prospects', 'openEuler & Friends 6.0'],
                    guest: [
                      {
                        name: 'Hu Xinwei',
                        post: 'Technical Committee Chairperson, OpenAtom openEuler',
                      },
                      {
                        name: 'Zheng Zhenyu',
                        post: 'Community Operations Team Lead, OpenAtom openEuler',
                      },
                    ]
                  },
                  {
                    title: 'Building a New Ecosystem for Scalable, Heterogeneous OSs',
                    name: 'Hu Chunming',
                    post: 'Dean, School of Software, Beihang University',
                  },
                ]
              },
              {
                time: '10:30 - 10:50',
                content: [
                  {
                    title: 'KylinSoft × China Post: Joint Innovation and Practice',
                    name: 'Gao Lijiang',
                    post: ' Head of Cloud Native Platform, China Post Information Technology (Beijing)',
                  },
                  {
                    title: 'Kylinsec × GAC Group: Joint Innovation and Practice',
                    guest: [
                      {
                        name: 'Ye Qinglong',
                        post: 'R&D Director, Kylinsec',
                      },
                      {
                        name: 'Chen Youxuan',
                        post: 'Head of HPC, GAC '
                      }
                    ]
                  },
                ]
              },
              {
                time: '10:50 - 11:00',
                content: [
                  {
                    title: '2025 openEuler Top Contribution Award',
                    name: 'Partner representatives',
                    post: '',
                  },
                  {
                    title: '2025 openEuler Project Star Award',
                    name: 'Project representatives',
                    post: '',
                  },
                  {
                    title: '2025 openEuler Contribution Star Award',
                    name: 'Developer representatives',
                    post: '',
                  },
                ]
              },
            ],
            tab: []
          },
          {
            title: 'Subforums',
            children: [],
            tab: [
              {
                label: 'Embedded & Embodied AI',
                time: '13:00 - 16:15',
                address: '',
                children: [
                  {
                    time: '13:00-13:15',
                    content: [
                      {
                        title: 'vPLCnext Powered by openEuler: Exploration and Practices',
                        name: 'Xu Peiwen',
                        post: 'Phoenix Contact',
                      }
                    ]
                  },
                  {
                    time: '13:15-13:30',
                    content: [
                      {
                        title: 'AI-powered Space-based openEuler Foundation for Satellite-borne Applications',
                        name: 'Wang Dawei',
                        post: 'JRLC',
                      }
                    ]
                  },
                  {
                    time: '13:30-13:45',
                    content: [
                      {
                        title: 'openEuler in Aerospace: Exploration and Practices',
                        name: 'Cai Xinqi',
                        post: 'Beijing Aerospace Microsystem and Information Technology Research Institute',
                      }
                    ]
                  },
                  {
                    time: '13:45-13:50',
                    content: [
                      {
                        title: 'Aerospace OS Launch',
                        name: '',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '13:50-14:05',
                    content: [
                      {
                        title: 'AimRT: A Programming Framework for Modern Robotics',
                        name: 'Hu Kuang',
                        post: 'AgiBot',
                      }
                    ]
                  },
                  {
                    time: '14:05-14:20',
                    content: [
                      {
                        title: 'AGIROS Common Algorithm Framework for Robotics',
                        name: 'Li Peng',
                        post: 'Institute of Software, Chinese Academy of Sciences',
                      }
                    ]
                  },
                  {
                    time: '14:20-14:35',
                    content: [
                      {
                        title: 'Industry Embodied AI Based on openEuler Embedded: Solution and Practices',
                        name: 'Wu Jian',
                        post: 'TD Tech',
                      }
                    ]
                  },
                  {
                    time: '14:35-14:50',
                    content: [
                      {
                        title: 'Next-gen Embodied AI Electronics Architecture',
                        name: 'Tan Rui',
                        post: 'Intewell',
                      }
                    ]
                  },
                  {
                    time: '14:50-15:05',
                    content: [
                      {
                        title: 'Robotics OS: Insights and Progress',
                        name: 'Xu Lei',
                        post: 'xFusion',
                      }
                    ]
                  },
                  {
                    time: '15:05-15:20',
                    content: [
                      {
                        title: 'Autonomous Mobile Robotics Based on All-homegrown Technologies',
                        name: 'Jiang Dongping',
                        post: 'Qino',
                      }
                    ]
                  },
                  {
                    time: '15:20-15:35',
                    content: [
                      {
                        title: 'QSemOS × IntarkDB Homegrown Embedded Foundational Software',
                        name: 'Liu Baoyu',
                        post: 'Greater Bay Area National Center of Technology Innovation',
                      }
                    ]
                  },
                  {
                    time: '15:35-15:',
                    content: [
                      {
                        title: 'Cross-node Multi-system Hybrid Deployment Using Declarative Orchestration',
                        name: 'Wei Jiangang',
                        post: 'HopeRun',
                      }
                    ]
                  },
                  {
                    time: '15:50-16:00',
                    content: [
                      {
                        title: 'Break',
                        name: '',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '16:00-16:15',
                    content: [
                      {
                        title: 'ROS SIG Development Plan and Robotic Practices',
                        name: 'Wang Xiaoyun',
                        post: 'Institute of Software, Chinese Academy of Sciences',
                      }
                    ]
                  },
                ],
              },
              {
                label: 'Developer Enablement & Tools',
                time: '13:00 - 15:50',
                address: '',
                children: [
                  {
                    time: '13:00-13:05',
                    content: [
                      {
                        title: 'Welcome & Opening Remarks',
                        name: 'Liu Fei',
                        post: 'Technical Expert, openEuler',
                      }
                    ]
                  },
                  {
                    time: '13:05-13:20',
                    content: [
                      {
                        title: 'OPEA and openHPC in openEuler Native Ecosystem',
                        name: 'Lu Weijun',
                        post: 'Senior Engineer, Huawei',
                      }
                    ]
                  },
                  {
                    time: '13:20-13:35',
                    content: [
                      {
                        title: 'DevStation: An AI-powered Developer Workstation',
                        name: 'Yuan Lipeng',
                        post: 'Senior Engineer, Huawei',
                      }
                    ]
                  },
                  {
                    time: '13:35-13:50',
                    content: [
                      {
                        title: 'Lightweight openEuler OS Customization, Build, and Deployment',
                        name: 'Liao Wei',
                        post: 'R&D Engineer, Kylinsec',
                      }
                    ]
                  },
                  {
                    time: '13:50-14:05',
                    content: [
                      {
                        title: 'Hot Patch Tool Based on LibcarePlus: Enhancements and Multi-architecture Multi-language Support',
                        name: 'Wu Heng',
                        post: 'R&D Expert, China Telecom eSurfing Cloud',
                      }
                    ]
                  },
                  {
                    time: '14:05-14:20',
                    content: [
                      {
                        title: 'Contributions and Improvements in openEuler Innovation Versions',
                        name: 'Funda Wang',
                        post: 'Individual Contributor',
                      }
                    ]
                  },
                  {
                    time: '14:20-14:35',
                    content: [
                      {
                        title: 'AI-powered Software Engineering Platform',
                        name: 'Guo Jiyan',
                        post: 'Director of Product R&D, Linx Software',
                      }
                    ]
                  },
                  {
                    time: '14:35-14:50',
                    content: [
                      {
                        title: 'AI-powered C2Rust Conversion Automation',
                        name: 'Zhang Haidong',
                        post: 'Manager of Technology R&D, UnionTech',
                      }
                    ]
                  },
                  {
                    time: '14:50-15:05',
                    content: [
                      {
                        title: 'AI Tooling in Document-based Development',
                        name: 'Ding Jiahui',
                        post: 'Senior Engineer, Huawei',
                      }
                    ]
                  },
                  {
                    time: '15:05-15:20',
                    content: [
                      {
                        title: 'AI LLM in Infrastructure',
                        name: 'Zhai Wenjie',
                        post: 'Senior Engineer, Huawei',
                      }
                    ]
                  },
                  {
                    time: '15:20-15:35',
                    content: [
                      {
                        title: 'PortGPT for LLM-based Automation of Patch Backporting',
                        name: 'Li Chaoyang',
                        post: 'Huazhong University of Science and Technology',
                      }
                    ]
                  },
                  {
                    time: '15:35-15:50',
                    content: [
                      {
                        title: 'LiteView: A Lightweight Browser Engine',
                        name: 'Xu Jingmin',
                        post: 'LiteView Kernel Architect, Institute of Software, Chinese Academy of Sciences',
                      }
                    ]
                  }
                ],
              },
              {
                label: 'Cloud Native',
                time: '13:00 - 15:35',
                address: '',
                children: [
                  {
                    time: '13:00-13:10',
                    content: [
                      {
                        title: 'Welcome & Opening Remarks',
                        name: 'Hu Xinwei  ',
                        post: 'Technical Committee Chairperson, OpenAtom openEuler',
                      }
                    ]
                  },
                  {
                    time: '13:10-13:30',
                    content: [
                      {
                        title: 'openYuanrong Serverless Distributed Computing Engine',
                        name: 'Liang Yi',
                        post: 'Chief Expert of General-Purpose Serverless, Huawei',
                      }
                    ]
                  },
                  {
                    time: '13:30-13:45',
                    content: [
                      {
                        title: 'Apache Doris Performance Optimization Based on Kunpeng',
                        name: 'Wu Zhiguo',
                        post: 'Engineer, Linaro',
                      }
                    ]
                  },
                  {
                    time: '13:45-13:55',
                    content: [
                      {
                        title: 'Challenges of AI Agent Sandbox vs. Traditional Cloud Native Architectures',
                        name: 'Fei Jianqiang',
                        post: 'Cloud Native Expert, Huawei',
                      }
                    ]
                  },
                  {
                    time: '13:55-14:05',
                    content: [
                      {
                        title: 'Conch: A Container Engine for Agentic AI Workloads',
                        name: 'Zhong Tao',
                        post: 'Engineer, Huawei',
                      }
                    ]
                  },
                  {
                    time: '14:05-14:20',
                    content: [
                      {
                        title: 'Real-time Topology Visualization and O&M of Cloud Native Networks',
                        name: 'Wan Feng',
                        post: 'Senior Software Engineer, KylinSoft',
                      }
                    ]
                  },
                  {
                    time: '14:20-14:40',
                    content: [
                      {
                        title: 'NPU Virtualization for Enhancing AI Server Resource Utilization',
                        name: 'Yang Bin',
                        post: 'Director, openEuler Research Center, iSoftStone',
                      }
                    ]
                  },
                  {
                    time: '14:40-15:00',
                    content: [
                      {
                        title: 'AI Agent Empowering Cloud Native Infrastructure',
                        name: 'Zhang Yinghui',
                        post: 'Cloud Native Architect, China Mobile Information',
                      }
                    ]
                  },
                  {
                    time: '15:00-15:15',
                    content: [
                      {
                        title: 'Cloud Native Immutable Infrastructure Based on CTyunOS',
                        name: 'Liu Min',
                        post: 'China Telecom eSurfing Cloud Technology',
                      }
                    ]
                  },
                  {
                    time: '15:15-15:35',
                    content: [
                      {
                        title: 'OmniRuntime for Big Data Acceleration: Opening Source and Practices',
                        name: 'Yin Weijie',
                        post: 'Big Data Expert, Huawei',
                      }
                    ]
                  }
                ],
              },
              {
                label: 'UnifiedBus',
                time: '13:00 - 15:40',
                address: '',
                children: [
                  {
                    time: '13:00-13:05',
                    content: [
                      {
                        title: 'Welcome & Opening Remarks',
                        name: 'Zhu Zhaosheng',
                        post: 'General Manager of Cluster Computing, Huawei',
                      }
                    ]
                  },
                  {
                    time: '13:05-13:25',
                    content: [
                      {
                        title: 'SuperPoD and Key Application Scenarios',
                        name: 'Zhang Jian',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '13:25-13:45',
                    content: [
                      {
                        title: 'CXL for Computer Architecture Restructuring: Position, Role, and Value',
                        name: 'Song Jiayu',
                        post: 'Secretary-General, CXL Tech Application Club',
                      }
                    ]
                  },
                  {
                    time: '13:45-14:00',
                    content: [
                      {
                        title: 'UnifiedBus Technology and Software Reference Design',
                        name: 'Niu Tao',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '14:00-14:15',
                    content: [
                      {
                        title: 'UnifiedBus Memory Pooling: Key Technologies and Applications',
                        name: 'Gao Chao',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '14:15-14:35',
                    content: [
                      {
                        title: 'UnifiedBus Communication: Key Technologies and Applications',
                        name: 'Li Lijun',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '14:35-14:50',
                    content: [
                      {
                        title: 'UnifiedBus Device Virtualization: Key Technologies and Applications',
                        name: 'Ye Biaoxiang',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '14:50-15:05',
                    content: [
                      {
                        title: 'SuperPoD Reliability: Key Technologies',
                        name: 'Deng Guangxing',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '15:05-15:25',
                    content: [
                      {
                        title: 'UnifiedBus High-level System Services: Key Technologies and Applications',
                        name: 'Yao Yiyu',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '15:25-15:40',
                    content: [
                      {
                        title: 'UnifiedBus SuperPoD Programming and Compiling: Key Technologies',
                        name: 'Yang Yang',
                        post: '',
                      }
                    ]
                  },
                ],
              },
              {
                label: 'AI',
                time: '13:00 - 16:00',
                address: '',
                children: [
                  {
                    time: '13:00-13:15',
                    content: [
                      {
                        title: 'Building an OS Development Team on openEuler Intelligence',
                        name: 'Sun Zhenghua',
                        post: 'R&D Director, Central Platform Dept, Sangfor',
                      }
                    ]
                  },
                  {
                    time: '13:15-13:30',
                    content: [
                      {
                        title: 'Next-gen Healthcare CDSS AI Engine Powered by openEuler',
                        name: 'Feng Yan',
                        post: 'Senior Engineer, Huimei Technology',
                      }
                    ]
                  },
                  {
                    time: '13:30-13:45',
                    content: [
                      {
                        title: 'LLM-based Tuning for Next-gen Core Trading Systems',
                        name: 'Huang Jun',
                        post: 'Core System Architect, China Merchants Securities',
                      }
                    ]
                  },
                  {
                    time: '13:45-14:00',
                    content: [
                      {
                        title: 'JuiceFS MLPerf Storage v2.0: Performance and Tuning',
                        name: 'Liu Xinliang ',
                        post: 'Engineer, Linaro',
                      }
                    ]
                  },
                  {
                    time: '14:00-14:15',
                    content: [
                      {
                        title: 'Intelligence BooM for Inference Optimization and Intelligent Applications',
                        name: 'Jin Long',
                        post: 'Solution Director, PowerLeader',
                      }
                    ]
                  },
                  {
                    time: '14:15-14:30',
                    content: [
                      {
                        title: 'Coordination of General and AI Computing Model Inference',
                        name: 'Li Yuzhen',
                        post: 'AI Engineer, Huawei',
                      }
                    ]
                  },
                  {
                    time: '14:30-14:45',
                    content: [
                      {
                        title: 'AI-powered O&M Platform',
                        name: 'Zhong Maoheng',
                        post: 'R&D Manager, Linx Software',
                      }
                    ]
                  },
                  {
                    time: '14:45-15:00',
                    content: [
                      {
                        title: 'TongTech Middleware AI Agent',
                        name: 'Li Jingjing',
                        post: 'Product Manager, TongTech',
                      }
                    ]
                  },
                  {
                    time: '15:00-15:15',
                    content: [
                      {
                        title: 'Ascend-optimized MoE EP Communication Acceleration Library',
                        name: 'Han Yuankun',
                        post: 'AI Communication Expert, Huawei',
                      }
                    ]
                  },
                  {
                    time: '15:15-15:30',
                    content: [
                      {
                        title: 'KunLun Ultimate Tuning Based on Ascend Soft FP8',
                        name: 'Sun Yanan',
                        post: 'Senior Engineer, Operator Development, KunLun Technology',
                      }
                    ]
                  },
                  {
                    time: '15:30-15:45',
                    content: [
                      {
                        title: 'ANNC: XLA-based Deep Learning Graph Compiler for Search and Recommendation',
                        name: 'Zheng Chenhui',
                        post: 'Software Engineer, Huawei',
                      }
                    ]
                  },
                  {
                    time: '15:45-16:00',
                    content: [
                      {
                        title: 'CTyunOS, an AI-powered Security Center',
                        name: 'Wu Kaishun',
                        post: 'Development Engineer, China Telecom eSurfing Cloud Technology',
                      }
                    ]
                  }
                ],
              },
              {
                label: 'Kernel & All-Scenario',
                time: '13:00 - 15:30',
                address: '',
                children: [
                  {
                    time: '13:00-13:15',
                    content: [
                      {
                        title: 'openEuler Kernel Community Developments',
                        name: 'Xie Xiuqi ',
                        post: 'openEuler Kernel SIG Maintainer',
                      }
                    ]
                  },
                  {
                    time: '13:15-13:30',
                    content: [
                      {
                        title: 'How to Copy Memory? Coordinated Asynchronous Copy as a First-Class OS Service',
                        name: 'Du Dongdong',
                        post: 'Assistant Professor, Shanghai Jiao Tong University',
                      }
                    ]
                  },
                  {
                    time: '13:30-13:45',
                    content: [
                      {
                        title: 'Break',
                        name: '',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '13:45-14:00',
                    content: [
                      {
                        title: 'AMD P-State: Balancing Performance and Power Efficiency',
                        name: 'Wang Wenkuan',
                        post: 'Linux Kernel Expert, AMD China',
                      }
                    ]
                  },
                  {
                    time: '14:00-14:15',
                    content: [
                      {
                        title: 'Power for Performance on Intel Platforms',
                        name: 'Zhang Rui',
                        post: 'Software Engineer, Intel',
                      }
                    ]
                  },
                  {
                    time: '14:15-14:30',
                    content: [
                      {
                        title: 'Programmable I/O Scheduler Design',
                        name: 'Cheng Kaitao',
                        post: 'Kernel Engineer, KylinSoft',
                      }
                    ]
                  },
                  {
                    time: '14:30-14:45',
                    content: [
                      {
                        title: 'trampoline and kexec_file Features Based on LoongArch',
                        name: 'Jiang Haoran',
                        post: 'R&D Engineer, KylinSoft',
                      }
                    ]
                  },
                  {
                    time: '14:45-15:00',
                    content: [
                      {
                        title: 'eBPF for Optimizing Multi-threaded System Information Collection',
                        name: 'Li Ziyao',
                        post: 'Senior R&D Engineer, UnionTech',
                      }
                    ]
                  },
                  {
                    time: '15:00-15:15',
                    content: [
                      {
                        title: 'eBPF-based Tool for Observing Memory, Process, I/O, and File Cache',
                        name: 'Song Zhen',
                        post: 'Senior Kernel Engineer, Kylinsec',
                      }
                    ]
                  },
                  {
                    time: '15:15-15:30',
                    content: [
                      {
                        title: 'UACCE Accelerator Framework for User-mode Innovations',
                        name: 'Feng Chengwen',
                        post: 'Engineer, Huawei',
                      }
                    ]
                  },
                ],
              },
              {
                label: 'Security & Confidential Computing',
                time: '13:00 - 15:35',
                address: '',
                children: [
                  {
                    time: '13:00-13:05',
                    content: [
                      {
                        title: 'Welcome & Opening Remarks',
                        name: 'Zhu Jianwei',
                        post: 'Chairperson, openEuler Security Committee',
                      }
                    ]
                  },
                  {
                    time: '13:05-13:20',
                    content: [
                      {
                        title: 'HAOC 3.0 Security Enhanced Hybrid Kernel Features and 4.0 Roadmap',
                        name: 'Wu Chenggang',
                        post: 'Researcher, Institute of Computing Technology, Chinese Academy of Sciences',
                      }
                    ]
                  },
                  {
                    time: '13:20-13:35',
                    content: [
                      {
                        title: 'Arm CCA for Open Source Confidential Computing',
                        name: 'Kaly Xin',
                        post: 'Director of Software Engineering, Linaro',
                      }
                    ]
                  },
                  {
                    time: '13:35-13:50',
                    content: [
                      {
                        title: 'Full-stack AI Security Technologies and Practices Based on openEuler',
                        name: 'Cheng Kun',
                        post: 'openEuler Confidential Computing SIG Maintainer',
                      }
                    ]
                  },
                  {
                    time: '13:50-14:05',
                    content: [
                      {
                        title: 'Confidential Computing for Dynamic Trusted Attestation and Confidential AI',
                        name: 'Andy',
                        post: 'Security Engineer, JD.com',
                      }
                    ]
                  },
                  {
                    time: '14:05-14:20',
                    content: [
                      {
                        title: 'AI-assisted CVE Vulnerability Remediation and Conflict Resolution',
                        name: 'Li Ping',
                        post: 'Senior Software Engineer, Huawei',
                      }
                    ]
                  },
                  {
                    time: '14:20-14:35',
                    content: [
                      {
                        title: 'AI-assisted Risk Management and Compliance Audit for Open Source Supply Chains',
                        name: 'Wang Lin',
                        post: 'Senior R&D Expert, China Unicom Digital Technology',
                      }
                    ]
                  },
                  {
                    time: '14:35-14:50',
                    content: [
                      {
                        title: 'Password Service Platform Based on Kunpeng Confidential Computing',
                        name: 'Zhang Chi',
                        post: 'Technology Director, Sansec',
                      }
                    ]
                  },
                  {
                    time: '14:50-15:05',
                    content: [
                      {
                        title: 'System Security Hardening Solution Built on openEuler',
                        name: 'Wang Yucheng',
                        post: 'R&D Engineer, Kylinsec',
                      }
                    ]
                  },
                  {
                    time: '15:05-15:20',
                    content: [
                      {
                        title: 'SBOM for Supply Chain Security Operations: Automation, Visualization, and openEuler Adaptation',
                        name: 'He Liyun',
                        post: 'Deputy General Manager, Linx Software Chengdu R&D Center',
                      }
                    ]
                  },
                  {
                    time: '15:20-15:35',
                    content: [
                      {
                        title: 'Database Performance Optimization Technology for TEE',
                        name: 'Lei Zhuoyi',
                        post: 'Master\'s Student, Lanzhou University',
                      }
                    ]
                  },
                ],
              },
              {
                label: 'RISC-V',
                time: '13:00 - 15:05',
                address: '',
                children: [
                  {
                    time: '13:00-13:05',
                    content: [
                      {
                        title: 'Welcome & Opening Remarks',
                        name: 'Wang Jingwei',
                        post: '',
                      }
                    ]
                  },
                  {
                    time: '13:05-13:25',
                    content: [
                      {
                        title: 'RISC-V User-mode Component Performance Optimization',
                        name: 'Lan Jing',
                        post: 'Performance Optimization Engineer, ZTE',
                      }
                    ]
                  },
                  {
                    time: '13:25-13:45',
                    content: [
                      {
                        title: 'RISC-V AI CPU Go-to-Market Explorations',
                        name: 'Chen Jiandong',
                        post: 'Product Director, SpacemiT',
                      }
                    ]
                  },
                  {
                    time: '13:45-14:05',
                    content: [
                      {
                        title: 'Ecosystem Evolution for RISC-V PMU Expansion and Performance Profiling',
                        name: 'Deng Tianmai',
                        post: 'Senior Software Engineer, UltraRISC',
                      }
                    ]
                  },
                  {
                    time: '14:05-14:25',
                    content: [
                      {
                        title: 'Open Source Practice of XuanTie C930 Virtualization and IOMMU on openEuler',
                        name: 'Yu Fangyu',
                        post: 'Kernel Technology Expert, Alibaba DAMO Academy',
                      }
                    ]
                  },
                  {
                    time: '14:25-14:45',
                    content: [
                      {
                        title: 'RISC-V Confidential Computing Explorations',
                        name: 'Zhang Jian',
                        post: 'Product Manager, BOSC',
                      }
                    ]
                  },
                  {
                    time: '14:45-15:05',
                    content: [
                      {
                        title: 'openEuler × SRA3 Server: RISC-V Architecture in Data Centers',
                        name: 'Wei Chao',
                        post: 'Embedded Software Engineer, OpenGPGPU',
                      }
                    ]
                  },
                ],
              }
            ]
          },
          {
            title: 'SIG Gathering',
            children: [],
            tab: [],
            list: [
              {
                title: 'SIG Gathering',
                children: [
                  {
                    label: 'Track ：Kernel & Basic Services',
                    time: '16:00 - 17:30',
                    address: '1F-105',
                    url: 'https://etherpad.openeuler.org/p/summit2025-sig-Kernel',
                    children: [],
                  },
                  {
                    label: 'Track ：Developer Enablement & Tools',
                    time: '16:00 - 17:30',
                    address: '1F-106',
                    url: 'https://etherpad.openeuler.org/p/summit2025-sig-community%26developer',
                    children: [],
                  },
                  {
                    label: 'Track ：AI Enablement & Application',
                    time: '16:00 - 17:30',
                    address: '1F-103',
                    url: 'https://etherpad.openeuler.org/p/summit2025-sig-ai',
                    children: [],
                  },
                  {
                    label: 'Track ：UnifiedBus',
                    time: '16:00 - 17:30',
                    address: 'G-G02',
                    url: 'https://etherpad.openeuler.org/p/summit2025-sig-UnifiedBus',
                    children: [],
                  },
                  {
                    label: 'Track ：Embedded & ROS',
                    time: '16:00 - 17:30',
                    address: 'G-G10A',
                    url: 'https://etherpad.openeuler.org/p/summit2025-sig-Embedded-ROS',
                    children: [],
                  },
                  {
                    label: 'Track ：RISC-V ',
                    time: '16:00 - 17:30',
                    address: 'G-G10B',
                    url: 'https://etherpad.openeuler.org/p/summit2025-sig-RISC-V',
                    children: [],
                  },
                  {
                    label: 'Track ：Security & Confidential Computing',
                    time: '16:00 - 17:30',
                    address: 'G-G01A',
                    url: 'https://etherpad.openeuler.org/p/summit2025-sig-gathering-ICC-NSG',
                    children: [],
                  },
                ]
              },
              {
                title: 'Evening Reception',
                children: [
                  {
                    label: 'Evening Reception',
                    time: '18:00 - 20:00',
                    address: '',
                    children: [],
                  }
                ]
              }
            ]
          }
        ]
      }
    ]
  },
  guest: {
    title: 'Speakers',
    list: [
      {
        title: 'Operating System Conference Keynote',
        children: [
          {
            name: 'Ni Guangnan',
            img: niguangnan,
            title: 'Academician\nChinese Academy of Engineering',
          },
          {
            name: 'Cheng Xiaoming',
            img: chengxiaoming,
            title: 'Chairman\nOpenAtom Foundation',
          },
          {
            name: 'Yang Chaobin',
            img: yangchaobin,
            title: 'Director, CEO of the ICT Business Group\nHuawei',
          },
          {
            name: 'Shao Guanglu',
            img: shaoguanglu,
            title: 'Director of Science and Technology Committee and Former General Manager\nChina Telecom Group',
          },
          {
            name: 'Dr. Xiong Wei',
            img: xiongwei,
            title: 'Chairperson\nOpenAtom openEuler Committee',
          },
          {
            name: 'Du Hai',
            img: duhai,
            title: 'General Manager of Hybrid Cloud\nBaidu AI Cloud',
          },
          {
            name: 'Zeng Wenbin',
            img: zengwenbin,
            title: 'Director of Operating System\nMeituan',
          },
          {
            name: 'Yang Mingming',
            img: yangmingming,
            title: 'Vice President\nPhoenix Contact China',
          },
        ]
      },
      {
        title: 'openEuler Summit 2025 Keynote',
        children: [
          {
            name: 'Ritchie Peng',
            img: penghonghua,
            title: 'Director of the ICT Strategy & Business Development Dept\nHuawei',
          },
          {
            name: 'Dr. Xiong Wei',
            img: xiongwei,
            title: 'Chairperson\nOpenAtom openEuler Committee',
          },
          {
            name: 'Tang Xiaolei',
            img: tangxiaolei,
            title: 'Vice President\nAMD',
          },
          {
            name: 'Andrew Wafaa',
            img: andrew,
            title: 'Arm Senior Director Software Communities & Fellow\nPyTorch Board',
          },
          {
            name: 'Hu Xinwei',
            img: huxinwei,
            title: 'Technical Committee Chairperson\nOpenAtom openEuler',
          },
          {
            name: 'Hu Chunming',
            img: huchunming,
            title: 'Dean, School of Software\nBeihang University',
          },
          {
            name: 'Grant Likely',
            img: grant,
            title: 'CTO\nLinaro',
          },
          {
            name: 'Lin Zhenfu',
            img: linzhenfu,
            title: 'Technology Expert\nCSG Digital Power Grid',
          },
          {
            name: 'Gao Lijiang',
            img: gaolijiang,
            title: 'Head of Cloud Native Platform\nChina Post Information Technology (Beijing)',
          },
          {
            name: 'Ye Qinglong',
            img: yeqinglong,
            title: 'R&D Director\nKylinsec',
          },
          {
            name: 'Chen Youxuan',
            img: chenyouxuan,
            title: 'Head of HPC\nGAC Group',
          },
        ]
      },
      {
        title: 'Technical Sessions Produced by',
        children: [
          {
            name: 'Du Dongdong',
            img: dudongdong,
            title: 'Assistant Professor\nShanghai Jiao Tong University',
          },
          {
            name: 'Lu Jun',
            img: lujun,
            title: 'Deputy Director, Lineage Engineering Technology Center, Guangzhou Institutes of Biomedicine and Health\nChinese Academy of Sciences ',
          },
          {
            name: 'Zhang Tao',
            img: zhangtao,
            title: 'Manager of Embedded Dept\nHopeRun',
          },
          {
            name: 'Jiang Haoran',
            img: jianghaoran,
            title: 'R&D Engineer\nKylinSoft',
          },
          {
            name: 'Song Shi',
            img: songshi,
            title: 'Chief Engineer\nGSK CNC Equipment',
          },
          {
            name: 'Yao Guangzhong',
            img: yaoguangzhong,
            title: 'Technology Expert\nHenan KunLun Technology',
          },
          {
            name: 'Yang Lu',
            img: yanglu,
            title: 'Director of PLCnext Software Application\nPhoenix Contact',
          },
          {
            name: 'Zhang Rui',
            img: zhangrui,
            title: 'Software Engineer\nIntel',
          },
          {
            name: 'Wang Wenkuan',
            img: wangwenkuan,
            title: 'Linux Kernel Expert\nAMD China',
          },
          {
            name: 'Song Jiayu',
            img: songjiayu,
            title: 'Secretary-General\nCXL Tech Application Club',
          },
          {
            name: 'Jin Xiaoxu',
            img: jinxiaoxu,
            title: 'Director of Chinese Enterprise Go Global\nYonyou',
          },
          {
            name: 'Zhang Xiao',
            img: zhangxiao,
            title: 'Director, Intelligent Computing Platform\niFLYTEK Spark AI Research Institute',
          },
          {
            name: 'Zhang Zehua',
            img: zhangzehua,
            title: 'Director, Ads Algorithm\nJD.com',
          },
          {
            name: 'Sun Zhenghua',
            img: sunzhenghua, 
            title: 'R&D Director, Central Platform Dept\nSangfor',
          },
          {
            name: 'Liu Bingwei',
            img: liubingwei,
            title: 'Senior Director, Software Technology Group\nIntel China',
          },
          {
            name: 'Cai Xinqi',
            img: caixinqi,
            title: 'Beijing Aerospace Microsystem and Information Technology Research Institute',
          },
          {
            name: 'Zeng Yi',
            img: zengyi,
            title: 'Director of Robotics Market\nTopstar',
          },
          {
            name: 'Ding Jiahui',
            img: dingjiahui,
            title: 'Senior Engineer\nHuawei',
          },
          {
            name: 'Fei Jianqiang',
            img: feijianqiang,
            title: 'Cloud Native Expert\nHuawei',
          },
          {
            name: 'Hu Kuang',
            img: hukuang,
            title: 'AgiBot',
          },
          {
            name: 'Jiang Dongping',
            img: jiangdongping,
            title: 'Qino',
          },
          {
            name: 'Jin Long',
            img: jinlong,
            title: 'Solution Director\nPowerLeader',
          },
          {
            name: 'Li Chaoyang',
            img: lichaoyang,
            title: 'Huazhong University of Science and Technology',
          },
          {
            name: 'Li Peng',
            img: lipeng,
            title: 'Institute of Software\nChinese Academy of Sciences',
          },
          {
            name: 'Liang Yi',
            img: liangyi,
            title: 'Serverless Chief Expert\nHuawei',
          },
          {
            name: 'Wu Jian',
            img: wujian,
            title: 'Chief Solutions Architect for Robotics\nTD Tech',
          },
          {
            name: 'Liu Baoyu',
            img: liubaoyu,
            title: 'Greater Bay Area National Center of Technology Innovation',
          },
          {
            name: 'Lu Weijun',
            img: luweijun,
            title: 'Senior Engineer\nHuawei',
          },
          {
            name: 'Tan Rui',
            img: tanrui,
            title: 'Intewell',
          },
          {
            name: 'Wan Feng',
            img: wanfeng,
            title: 'Senior Software Engineer\nKylinSoft',
          },
          {
            name: 'Wang Dawei',
            img: wangdawei,
            title: 'JRLC',
          },
          {
            name: 'Wang Wei',
            img: wangwei,
            title: 'Head of Unmanned Equipment\nJRLC',
          },
          {
            name: 'Wang Xiaoyun',
            img: wangxiaoyun,
            title: 'Institute of Software\nChinese Academy of Sciences',
          },
          {
            name: 'Wang Zhiqiang',
            img: wangzhiqiang,
            title: 'Head of Marketing\nCODESYS Software Group',
          },
          {
            name: 'Wei Jiangang',
            img: weijiangang,
            title: 'HopeRun',
          },
          {
            name: 'Wu Heng',
            img: wuheng,
            title: 'R&D Expert\nChina Telecom eSurfing Cloud',
          },
          {
            name: 'Wu Zhiguo',
            img: wuzhiguo,
            title: 'Engineer\nLinaro',
          },
          {
            name: 'Xu Lei',
            img: xulei,
            title: 'xFusion',
          },
          {
            name: 'Xu Peiwen',
            img: xupeiwen,
            title: 'Phoenix Contact',
          },
          {
            name: 'Yang Bin',
            img: yangbin,
            title: 'Director, openEuler Research Center\niSoftStone',
          },
          {
            name: 'Yuan Lipeng',
            img: yunlipeng,
            title: 'Senior Engineer\nHuawei',
          },
          {
            name: 'Zhai Wenjie',
            img: zhuowenjie,
            title: 'Senior Engineer\nHuawei',
          },
          {
            name: 'Zhong Maoheng',
            img: zhongmaoheng,
            title: 'Manager\nLinx Software',
          },
          {
            name: 'Zhong Tao',
            img: zhongtao,
            title: 'Engineer\nHuawei',
          },
          {
            name: 'Liu Xinliang',
            img: liuxinliang,
            title: 'Engineer\nLinaro',
          },
          {
            name: 'Andy',
            img: andy,
            title: 'Security Engineer\nJD.com',
          },
          {
            name: 'He Liyun',
            img: heliyun,
            title: 'Vice General Manager of R&D Center\nBeijing Linx Software Corp.',
          },
          {
            name: 'Emma',
            img: liping,
            title: 'Senior Software Engineer\nHuawei',
          },
          {
            name: 'Leon Wang',
            img: wanglin,
            title: 'Senior R&D Expert\nChina Unicom Digital Technology',
          },
          {
            name: 'Wang Yucheng',
            img: wangyucheng,
            title: 'Engineer',
          },
          {
            name: 'Wu Chenggang',
            img: wuchenggang,
            title: 'Professor\nInstitute of Computing Technology, CAS/ZGC Lab',
          },
          {
            name: 'Kaly Xin',
            img: xinxiaohui,
            title: 'Software Engineering Director\nLinaro',
          },
          {
            name: 'Charles Zhang',
            img: zhangchi,
            title: 'Director\nSansec Technology',
          },
          {
            name: 'Feng Yan',
            img: fengyan,
            title: 'Senior Java Backend Engineer\nHuimei Cloud Technology',
          },
          {
            name: 'Han Yuankun',
            img: hanyuankun,
            title: 'AI Communication Expert\nHuawei',
          },
          {
            name: 'Kevin',
            img: huangjun,
            title: 'Core System Architect\nChina Merchants Securities',
          },
          {
            name: 'Li Jingjing',
            img: lijingjing,
            title: 'Product Manager\nBeijing TongTech',
          },
          {
            name: 'Li Yuzhen',
            img: liyuzhen,
            title: 'AI Engineer\nHuawei',
          },
          {
            name: 'Sun Yanan',
            img: sunyanan,
            title: 'Senior Operator Development Engineer\nHenan KunLun Technology',
          },
          {
            name: 'Zheng Chenhui',
            img: zhegnchenhui,
            title: 'Engineer\nHuawei',
          },
        ]
      },
      {
        title: 'Technical Sessions',
        children: [
          {
            name: 'Xie Xiuqi',
            img: xiexiuqi,
            title: 'openEuler Kernel SIG Maintainer',
          },
          {
            name: 'Luo Yukai',
            img: luoyukai,
            title: 'Security Committee Member\nopenEuler',
          },
          {
            name: 'Du Kaitian',
            img: dukaitian,
            title: 'Maintainer\nopenEuler',
          },
          {
            name: 'Zhu Tao',
            img: zhutao,
            title: 'openEuler\nsig-UnifiedBus Maintainer',
          },
          {
            name: 'Ren Wei',
            img: renwei,
            title: 'openEuler Technical Committee\nMember',
          },
          {
            name: 'Li Yang',
            img: liyang,
            title: 'Distributed Middleware SIG Maintainer',
          }
        ]
      }
    ]
  },
  partner: {
    title: 'Sponsors',
    list: [
      {
        title: 'Hosts',
        logos: [
          {
            light: openEuler,
            dark: openEulerDark
          }
        ]
      },
      {
        title: 'Co-Hosts',
        logos: [
          {
            light: huawei,
            dark: huawei_dark
          },
          {
            light: qiling,
            dark: qiling_dark
          },
          {
            light: xinan,
            dark: xinan_dark
          }
        ]
      },
      {
        title: 'Co-Organizers',
        logos: [
          {
            light: ningsi,
            dark: ningsi_dark
          },
          {
            light: xfusion,
            dark: xfusion_dark
          },
          {
            light: dongfang,
            dark: dongfang_dark
          },
          {
            light: ruantong,
            dark: ruantong_dark
          },
          {
            light: runhe,
            dark: runhe_dark
          }
        ]
      },
      {
        title: 'Supporter',
        logos: [
          {
            light: baolande,
            dark: baolande_dark
          },
          {
            light: tongxin,
            dark: tongxin_dark
          },
        ]
      }
    ]
  },
  previous: {
    title: '',
    list: [],
  },
};
